...
import type { MessageCallbackEntry, PluginMessageAPI } from "./types";
		...
		for (const entry of this._onMessageCallback) {
			try {
				...
				if (result instanceof Promise) {
					...
					result.catch((e) => console.error("Plugin message callback error:", e));
				}
			} catch (e) {
				console.error("Plugin message callback error:", e);
			}
		}
	}
